...
HttpServer::HttpServer(const std::string &recordingsPath, uint16_t port)
        : recordingsPath_(recordingsPath), web_static_dir_(recordingsPath), port_(port), shutdownPromise_{}, shutdownFuture_{shutdownPromise_.get_future()} {}

HttpServer::HttpServer(const std::string &recordingsPath, const std::string &web_static_dir, uint16_t port)
        : recordingsPath_(recordingsPath), web_static_dir_(web_static_dir), port_(port), shutdownPromise_{}, shutdownFuture_{shutdownPromise_.get_future()} {}
...
void HttpServer::startHttpServer() {
    ...
    std::cout << "Serving files from " << web_static_dir_ << std::endl;
    serverThread_ = std::thread(&HttpServer::runServer, this);
}
...
void HttpServer::runServer() {
    ...
    // Start Drogon in the current thread
    std::thread([this]() {
        drogon::app().run();
    }).detach();
...
void HttpServer::stop() {
    // Signal shutdown if not already triggered
    if (shutdownFuture_.valid()) {
        shutdownPromise_.set_value();  // tell the server to quit
    }
